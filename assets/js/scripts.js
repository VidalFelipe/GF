...
function antlerSettings({
  ...
  this.layout =
    this.showSettings === "show"
      ? getCookie("layout") || "wide"
      : layout || "wide";
  this.color =
    this.showSettings === "show"
      ? getCookie("color") || "pink"
      : color || "pink";
  this.background =
    this.showSettings === "show"
      ? getCookie("background") || "dark"
      : background || "dark";
  this.font =
    this.showSettings === "show"
      ? getCookie("font") || "opensans"
      : font || "opensans";
  this.header =
    this.showSettings === "show"
      ? getCookie("header") || "static"
      : header || "static";
  this.textDirection =
    this.showSettings === "show"
      ? getCookie("textDirection") || "ltr"
      : textDirection || "ltr";
  this.radius =
    this.showSettings === "show"
      ? getCookie("radius") || "defaultradius"
...
/*---------------------------------*/
function settings() {
  $("#settings").load("settings.html", function (res, status, xhr) {
    changeColorStyle();
  });
}
/*---------------------------------*/
    ...
    dToggle.on("click", function () {
      ...
      if (planPeriod.hasClass("annually")) {
        ...
        for (var i = 0; i <= 9; i++) {
          ...
          $(".price-container:eq(" + i + ") .discount").text(
            parseFloat(
              Number($(".price-container:eq(" + i + ") .discount").text()) / 12
            ).toFixed(2)
          );
        }
      } else {
  ...
  $window
    .bind("scroll resize", function () {
      ...
      if (scrollTop < $content.offset().top + -35) {
        ...
        scrollTop >
        $content.offset().top + $content.outerHeight(true) - $el.height() - 165
      ) {
        $el.css({
          top: "auto",
          bottom:
            $window.height() +
            scrollTop -
            ($content.offset().top + $content.outerHeight(true)) +
            "px",
        });
      } else {
        $el.css({
